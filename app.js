...
const MIN=62,MAX=105; let lo=MIN,hi=MAX,filtered=D.slice(),active=null,photoData=null,statusFilter='';
...
function step(){let span=hi-lo;return span>32?5:span>18?2:span>7?1:span>3?.5:.2}
...
function drawLane(id,side){let lane=$(id);lane.innerHTML='<div class="lane-label">'+side+'</div><div class="release-uncovered"></div>';let span=hi-lo;R.filter(x=>x.lado===side&&x.kf>=lo&&x.ki<=hi).forEach(x=>{let a=Math.max(lo,x.ki),b=Math.min(hi,x.kf);if(b<=a)return;let d=document.createElement('div');d.className='release-green';d.style.left=((a-lo)/span*100)+'%';d.style.width=((b-a)/span*100)+'%';lane.appendChild(d)})}
function render(){
 let axis=$('axis');
 axis.querySelectorAll('.tick,.item').forEach(e=>e.remove());
 ...
 drawLane('laneLE','LE'); drawLane('laneLD','LD');
 let groups={LE:[],LD:[],center:[]};
 filtered.forEach(r=>{
   let side=String(val(r,'LADO')).trim().toUpperCase();
   side=side==='LE'?'LE':side==='LD'?'LD':'center';
   let a=Math.max(lo,+r._ki), b=Math.min(hi,+r._kf);
   if(b<=a)return;
   groups[side].push({r,a,b});
 });
 Object.entries(groups).forEach(([side,arr])=>arr.forEach((o,i)=>{
   let d=document.createElement('button');
   d.type='button';
   d.className='item '+side.toLowerCase();
   d.title=String(val(o.r,'IDENTIFICAÇÃO'));
   d.setAttribute('aria-label',String(val(o.r,'IDENTIFICAÇÃO')));
   d.style.left=((o.a-lo)/span*100)+'%';
   d.style.width=Math.max((o.b-o.a)/span*100,0.35)+'%';
   d.style.background=color(o.r);
   d.style.top=(side==='center'?68:58+(i%4)*27)+'px';
   d.onclick=function(){openModal(o.r)};
   axis.appendChild(d);
 }));
}
function renderLegend(){let box=$('legend'),tp=$('tipo').value;box.innerHTML='';if(!tp){box.innerHTML='<b>Tipos:</b> <span><i class="sw" style="background:#2563eb"></i>OAE</span><span><i class="sw" style="background:#16a34a"></i>CONTENÇÃO</span><span><i class="sw" style="background:#f59e0b"></i>TFA</span>';return}let sols=[...new Set(filtered.map(r=>String(val(r,'SOLUÇÃO')).trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'));box.innerHTML='<b>'+esc(tp)+':</b>';sols.forEach((s,i)=>{let sp=document.createElement('span');sp.innerHTML='<i class="sw" style="background:'+shade(COLORS[tp]||'#64748b',[-40,-20,0,20,40,60,75][i%7])+'"></i>'+esc(s);box.appendChild(sp)})}
function renderTable(){let tb=$('tbody');tb.innerHTML='';if(!filtered.length){tb.innerHTML='<tr><td colspan="12" class="empty">Nenhuma frente encontrada.</td></tr>';return}filtered.forEach(r=>{let s=storageGet('frente_'+key(r)),obs=s.obs||val(r,'OBSERVAÇÃO'),photo=s.photo||val(r,'FOTO');let tr=document.createElement('tr');tr.innerHTML='<td>'+(filtered.indexOf(r)+1)+'</td><td>'+fmt(r._ki)+'</td><td>'+fmt(r._kf)+'</td><td>'+esc(val(r,'IDENTIFICAÇÃO'))+'</td><td>'+esc(val(r,'TRECHO'))+'</td><td>'+esc(val(r,'TIPO'))+'</td><td>'+esc(val(r,'SOLUÇÃO'))+'</td><td>'+esc(val(r,'LADO'))+'</td><td>'+esc(val(r,'EXTENSÃO'))+'</td><td><span class="status '+(released(r)?'yes':'no')+'">'+(released(r)?'LIBERADA':'NÃO LIBERADA')+'</span></td><td>'+esc(obs||'')+'</td><td>'+(photo?'📷':'')+'</td>';tr.onclick=()=>openModal(r);tb.appendChild(tr)})}
